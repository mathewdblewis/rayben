        ...
        for index in range(start, start+count):
            ...
            # if file['scales/write_number'][index]%1==0:
                ...
                for n, task in enumerate(tasks):
                    ...
                    plot_tools.plot_bot_3d(dset, 0, index, axes=axes, title='temperature field', cmap='bwr')
                
                # Add time and parameter title
                Ra  = file['tasks']['myRa'] [index][0][0]
                Pr  = file['tasks']['myPr'] [index][0][0]
                Lx  = round(file['tasks']['myLx'] [0][0][0],2)
                Res = round(file['tasks']['myRes'][0][0][0],2)
                t   = file['scales/sim_time'][index]
                title = 'Ra=%.2E, Pr=%.2E, Lx=%.2E, res=%.2E, t=%.2E' % (Ra,Pr,Lx,Res,t)
                title_height = .3 - 0.5 * mfig.margin.top / mfig.fig.y
